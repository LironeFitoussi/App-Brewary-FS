...
class Joke {
  constructor(id, jokeText, jokeType) {
    this.id = id,
...
app.post("/jokes", (req, res) => {
  ...
  console.log(`New Joke Posted at ${new Date}: \n ${JSON.stringify(jokes.slice(-1))}`);
});
...
app.patch("/jokes/:id", (req, res) => {
  ...
  const updatedJoke = new Joke (
    id,
    req.body.jokeText || exisitingJoke.jokeText,
    req.body.jokeType || exisitingJoke.jokeType
  )
...
app.delete("/all", (req, res) => {
  ...
  if (userKey === masterKey) {
    jokes = [];
    res.sendStatus(200).json({ message: "All jokes deleted successfully." });
  } else {
    res
      .sendStatus(404)
      .json({error: "You are not authorised to perform this actrion."})
  }
});
app.listen(port, () => {
